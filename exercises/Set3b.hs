...
sums :: Int -> [Int]
sums i = sums' 1 0
         where sums' curr runSum = if curr > i
                                       then []
                                       else nextSum : sums' (curr+1) nextSum
                                           where nextSum = runSum + curr
...
indexDefault xs i def = if i < 0
                           then def
                           else indexDefault' xs i def
                        where indexDefault' [] _ def = def
                              indexDefault' (x:li) 0 def = x
                              indexDefault' (x:li) n def = indexDefault' li (n-1) def
...
sumsOf xs = sums' xs 0
            where sums' [] runSum = []
                  sums' (c:cs) runSum = nextSum:sums' cs nextSum
                      where nextSum = runSum + c
