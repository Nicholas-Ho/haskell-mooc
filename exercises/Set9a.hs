...
countValid [] = 0
countValid (x:xs) = let valid = if (x !! 2) == (x !! 4) || (x !! 3) == (x !! 5) then 1 else 0
                    in valid + countValid xs
...
repeated [] = Nothing
repeated (x:xs) = let repeated'  _   []     = Nothing
                      repeated' num (y:ys) = if num == y
                                               then Just num
                                               else repeated' y ys
                  in repeated' x xs
...
sumSuccess (x:xs) = case x of
                      (Left _)   -> next
                      (Right n1) -> case next of
                                      (Left _)   -> Right n1
                                      (Right n2) -> Right $ n1 + n2
                    where next = sumSuccess xs
...
open attempt lo = case lo of
                    (Locked code) | code == attempt -> Unlocked code
                                  | otherwise       -> lo
                    _ -> lo
...
instance Eq Text where
  (==) (Text s1) (Text s2) = rmWs s1 == rmWs s2
                             where rmWs = foldr (\c ns -> if isWs c then ns else c:ns) ""
                                   isWs ' '  = True
                                   isWs '\n' = True
                                   isWs _    = False
...
compose l1 l2 = foldr process [] l1
                where process e1 res = case lookup (snd e1) l2 of
                                         (Just v) -> (fst e1, v):res
                                         Nothing  -> res
...
permute p l = map (\i -> case elemIndex i p of (Just index) -> l !! index) (identity (length p))
