...
member x (Set l) = foldr (\a b -> a == x || b) False l

-- add a member to a set
add :: (Ord a, Eq a) => a -> Set a -> Set a
add x s = if member x s
             then s
             else Set $ insertSorted x (extractList s)
          where extractList (Set l) = l
                insertSorted x [] = [x]
                insertSorted x (y:ys) = if y < x
                                           then y:insertSorted x ys
                                           else x:y:ys
...
data State = Start | Error | Finished | EggsAdded | FlourAdded | SugarAdded | MixReady | Mixed
  deriving (Eq,Show)

step Start e = case e of
                 AddEggs -> EggsAdded
                 _ -> Error
step EggsAdded e = case e of
                 AddFlour -> FlourAdded
                 AddSugar -> SugarAdded
                 _ -> Error
step FlourAdded e = case e of
                 AddSugar -> MixReady
                 _ -> Error
step SugarAdded e = case e of
                 AddFlour -> MixReady
                 _ -> Error
step MixReady e = case e of
                 Mix -> Mixed
                 _ -> Error
step Mixed e = case e of
                 Bake -> Finished
                 _ -> Error
step state _ = state -- Finished and Error
...
reverseNonEmpty xs = go xs []
                     where nonEmpty (y:ys) = y :| ys
                           go (y :| ys) res = if length ys == 0
                                                 then y :| res
                                                 else go (nonEmpty ys) (y:res)
...
instance (Ord a, Eq a) => Semigroup (Set a) where
  (<>) (Set l1) set = go l1 set
    where go [] s = s
          go (x:xs) s = go xs (add x s)
...
passwordAllowed pw (ContainsSome s) = any id $ map (\c -> any (==c) s) pw
...
data Arithmetic = Literal Integer | Add Arithmetic Arithmetic | Subtract Arithmetic Arithmetic | Multiply Arithmetic Arithmetic
  deriving Show
...
evaluate a = case a of
               ...
               (Subtract a1 a2) -> evaluate a1 - evaluate a2
...
render a = case a of
               ...
               (Subtract a1 a2)  -> "(" ++ render a1 ++ "-" ++ render a2 ++ ")"
