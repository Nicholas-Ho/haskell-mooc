...
distinct li = length li == length (nub li)
...
rangeOf li = last sortedList - head sortedList
             where sortedList = sort li
...
longest :: (Ord a) => [[a]] -> [a]
longest = last . sortBy (check (comparing length))
      where check f x y = case f x y of
                            EQ  -> comparing head y x  -- note flip
                            ord -> ord
...
incrementKey key = map (\(a, b) -> if a == key then (a, b+1) else (a, b))
...
freqs li = foldr (\x b -> Map.alter f x b) Map.empty li
               where f v = case v of
                             Nothing      -> Just 1
...
transfer from to amount bank = if Map.notMember to bank || amount < 0
                                  then bank
                                  else case Map.lookup from bank of
                                         Nothing -> bank
                                         Just mon -> if mon < amount
                                                        then bank
...
maxIndex ar = fst . last . sortBy (comparing snd) $ assocs ar
