...
instance Eq a => Eq (List a) where
  (==) (LNode x xn) (LNode y yn) = x == y && xn == yn
  (==) Empty        Empty        = True
  (==) _            _            = False
...
instance Ord Number where
  ...
  compare (Finite i) (Finite j) = compare i j
...
simplify (RationalNumber i j) = RationalNumber (i `div` d) (j `div` d)
                                  where d = gcd i j
...
instance Num RationalNumber where
  (+) (RationalNumber n1 d1) (RationalNumber n2 d2) = simplify $ RationalNumber n d
                                                        where n = n1 * d2 + n2 * d1
                                                              d = d1 * d2
  (*) (RationalNumber n1 d1) (RationalNumber n2 d2) = simplify $ RationalNumber n d
                                                        where n = n1 * n2
  ...
  signum (RationalNumber n d) | (n < 0 && d < 0) || (n > 0 && d > 0) = RationalNumber 1 1
